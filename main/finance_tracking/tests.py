...
class TestFinanceTrackingModels(TestCase):
    ...
    # Test for category name too long
    def test_category_name_too_long(self):
        with self.assertRaises(ValidationError) as context:
            Category.objects.create(name='t' * 61, group=DEPOSIT, user=self.user)
        self.assertTrue('Name cannot exceed 60 characters, please try again!' in str(context.exception))
